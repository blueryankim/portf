					...
					<c:if test="${count != 0  }">
						<c:forEach var="vo" items="${boardList }">
							<tr>
								...
								<td align="left"><a
									href="content.do?num=${vo.num}&pageNum=${bp.cur_Page}">
										&nbsp;${vo.subject}</a> <c:if test="${vo.readcount > 10 }">
	...
	<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%">
		...
		<h3>Q&A 게시판</h3>
				...
				<table width="700">
					<tr>
						<td align="right" colspan="5"><input type="button"
							value="전체목록" onclick="window.location='list.do'" /> <input
				...
				<c:forEach var="counter" begin="${bp.getPage_Start() }"
					...
					<c:if test="${search_text ne ''}">
						<a
							href="javascript:window.location='list.do?pageNum=${counter }&search_type=${search_type}&search_text=${search_text}'">
							[${counter}]</a>
					</c:if>
					<c:if test="${search_text eq ''}">
